...
The research question is that "Can we reconstruct the lineage of how cells differentiate?". [McKenna et al Science (2016)](https://science.sciencemag.org/content/353/6298/aaf7907) shows the possibility that lineage can be reconstructed through the edited barcode of CRISPR/Cas9 target sites. Scientists can reconstruct the cell lineange based on the information from barcodes of each cell.
...
With Neighbor Joining or fastme by [Desper, R. and Gascuel, O. (2002)](https://www.ncbi.nlm.nih.gov/pubmed/12487758), we can construct tree from the distance matrix.
...
We can estimate mutation probability for each outcome state from the data by their frequencies. Less frequent outcomes are less likely to observe. Less likely outcomes would corresponds with farther distances. Thus one idea for outcome states is with their information entropy. $w_a = -\log $P(base is A$)$, \cdots, w_z = -\log $P(base is Z).$
